        ...
        <aside className="sidebar">
            ...
            <div className="categories pt__60">
                ...
                <ul>
                    {categories.map((category, index) => (
                        <li key={index}>
                            <Link to="/blog-details">{category.name}</Link>
                            <p>({category.count})</p>
                        </li>
                    ))}
                </ul>
                ...
                <ul>
                    {
                        blogOneData.slice(0, 3).map(({ id, title, imgSrc }) => {
                            return (
                                <li key={id}>
                                    <a href>
                                        <img src={imgSrc} alt className="thumb__img" />
                                    </a>
                                    <div>
                                        <Link to="#">
                                            {title}
            ...
            <div className="tags pt__60">
                ...
                <ul>
                    {tags.map((tag, index) => (
                        <li key={index}>
                            <Link to="#" className={tag === "Design" ? "active" : ""}>{tag}</Link>
                        </li>
                    ))}
                </ul>
            </div>
